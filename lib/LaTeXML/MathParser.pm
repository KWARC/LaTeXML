...
sub ApplyNary {
  ...
  if (p_getQName($arg2) eq 'ltx:XMApp') {
    ...
    my $rop2 = realizeXMNode($op2);
    if (((p_getTokenMeaning($rop2) || '__undef_meaning__') eq $opname)    # Same operator?
      && ((p_getValue($rop2) || '__undef_content__') eq $opcontent)
      # Check that ops are used in same way.
      && !grep((p_getAttribute($rop, $_) || '<none>') ne (p_getAttribute($rop2, $_) || '<none>'),
        qw(mathstyle))    # Check ops are used in similar way
